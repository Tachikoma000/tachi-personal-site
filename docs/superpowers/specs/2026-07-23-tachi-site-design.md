...
- (2026-07-26, **site-wide contrast audit**, after the chrome fix below): every text node on the arrival, poems index, both family pages, a poem with a note, The Knock, a book, and about was measured at all six hours against its *actual composited background* (the sky gradient interpolated at that element's y-position, over paper) — not against `--paper`, which is what makes these failures invisible to a naive check. It found two further classes of failure and both are fixed. (a) **`--accent` is a text colour** — store links, piece meta, note labels, family counts — and at dawn/morning/dusk it measured 3.1–3.7 on paper, failing AA: dawn `#B8797E → #805054`, morning `#6E89A8 → #576D86`, dusk `#A9713F → #82572F`. Each keeps its hue, only deepens, and each still clears 3:1 as the active-nav underline over its own sky. (b) `--ink-soft` `#5B5963 → #4E4C56`, because `.section-note` sits just under the title where the sky is still strong and measured 3.9. Softness is preserved — ink 11.3 vs soft 7.6 on paper. **Result: all page types clean at all six hours.** When adding an hour or an accent, check the accent as *text on paper* (≥4.5) and as *underline on its own sky* (≥3), and remember chrome is measured against the sky, not the paper.
- (2026-07-26, **legibility of the chrome** — measured, not eyeballed): the navbar and other faint controls sit over the **sky gradient**, not over paper, so their contrast changed with the hour. Measured `--ink-soft`/300 against each hour's sky-top: dawn 4.32, midday 3.76, dusk 3.45 all failed WCAG AA, and **l'heure bleue measured 1.07 — effectively invisible**. Two fixes. (1) Interactive chrome — nav links, home word, hour mark, back links, skip-note and bridge, next-poem label, arrival path hints — moved from `--ink-soft`/300 to `--ink`/400, roughly doubling contrast at every hour. Body text on paper was left alone; it already measured AA and the quiet look depends on it. (2) **l'heure bleue's palette was corrected**: its sky-top was ~4× darker than any other light-theme hour (so dark ink failed at any weight) and its accent was *the same colour as its sky*, giving the active-nav underline 1.00 contrast. Sky-top `#46557E → #9AA3BE`, sky-mid `#8B90AE → #C2C6D6` (keeping the gradient brightening downward), accent `#46557E → #3C4A72`. All six hours now pass AA for chrome; bleue improved 4.7×.
- (2026-07-26, **5e · the telling, set** — reshaped from `docs/design/2026-07-26-decisions.md` §7): a telling's **first stanza renders 1.12em**, so a long praise-song opens its throat and then speaks. **Desktop only, tellings only.** The original asked for a 1.5rem opening over a 1.32rem body, but 1.5rem is now the desktop ceiling of the poem's fluid clamp — honouring it literally would have shrunk every telling's reading size and done nothing at all on mobile. Instead the body keeps its tuned size and the opening lifts above it, expressed in `em` so it rides the clamp. Mobile is deliberately untouched: enlarging an opening stanza there would undo the line-wrap fix (verified unchanged at 6%). Breaths are excluded — often three stanzas, so enlarging the first would enlarge most of the poem.
